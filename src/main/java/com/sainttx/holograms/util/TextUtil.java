...
public class TextUtil {
    ...
    public static ItemStack parseItem(String text) {
        ...
        if (meta instanceof Damageable) {
            ((Damageable) meta).setDamage((short) Math.max(0, durability));
        }
        ...
        for (int i = 2 ; i < split.length ; i++) {
            ...
            String s = information[0].toLowerCase();
            if ("name".equals(s)) {// Replace '_' with spaces
                String name = information[1].replace(' ', ' ');
                meta.setDisplayName(ChatColor.translateAlternateColorCodes('&', name));

            } else if ("lore".equals(s)) {// If lore was specified 2x for some reason, don't overwrite
            ...
            } else {// Try parsing enchantment if it was nothing else
                Enchantment ench = Enchantment.getByName(information[0].toUpperCase());
                int level = Integer.parseInt(information[1]);
